    ...
    public partial class frmZaposleniciUnos : Form
    {
        ...
        private async void btnSacuvaj_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtSifra.Text) ||
                ...
                (rbtnM.Checked==false && rbtnZ.Checked==false)
                )
            {
            ...
            {
                MessageBox.Show("Sifra može imati najviše 3 karaktera", "Upozorenje", MessageBoxButtons.OK);
                return;
            }
            else if (!string.IsNullOrWhiteSpace(txtSifra.Text))
            {
                List<Models.zaposlenici> t = await service.Get<List<Models.zaposlenici>>(null);
                foreach(var x in t)
                {
                    if (x.Sifra == txtSifra.Text)
                    {
                        MessageBox.Show("Unesena šifra se mora promijeniti", "Upozorenje", MessageBoxButtons.OK);
                        postoji = true;
                        break;
                    }
                }
            ...
            if (!string.IsNullOrWhiteSpace(txtSifra.Text) &&
               ...
               !string.IsNullOrWhiteSpace(txtIme.Text) &&
               !string.IsNullOrWhiteSpace(txtPrezime.Text) &&
               (rbtnM.Checked == true || rbtnZ.Checked == true) &&
               !postoji
               )
